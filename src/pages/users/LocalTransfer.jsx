    ...
    <div className="">
      <div className="flex flex-col">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-md">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-slate-100 mb-2">United Save Capital Secure Local Transfer</h2>
            ...
            <div className="text-center py-8">
              ...
              {formData.description && (
                <div className="bg-gray-50 dark:bg-slate-700 p-4 rounded-lg max-w-md mx-auto mb-6">
                  ...
                  <p className="text-gray-800 dark:text-slate-200">{formData.description}</p>
                </div>
              )}
              <button
                onClick={resetForm}
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="font-medium text-gray-800 dark:text-slate-100">
                          {formData.selectedAccount === "saving" 
                            ? `SAVINGS - ${wallet?.savingAccountNumber} (${wallet?.currency}${wallet?.saving?.toLocaleString('en-US') || 0})` 
                            : `CHECKING - ${wallet?.checkingAccountNumber} (${wallet?.currency}${wallet?.checking?.toLocaleString('en-US') || 0})`}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500 dark:text-slate-400">Beneficiary</p>
                        <p className="font-medium text-gray-800 dark:text-slate-100">{formData.beneficiaryName}</p>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <div>
                        <p className="text-sm text-gray-500 dark:text-slate-400">Bank Details</p>
                        <p className="font-medium text-gray-800 dark:text-slate-100">{formData.bankName}</p>
                        <p className="text-sm text-gray-600 dark:text-slate-300">{formData.accountNumber}</p>
                      </div>
                    </div>
                  </div>
                  {formData.description && (
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-5 rounded-xl border border-blue-100 dark:border-blue-800/30">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-gray-800 dark:text-slate-100">£{parseFloat(formData.amount).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600 dark:text-slate-400">Fee (1%)</span>
                      <span className="font-medium text-gray-800 dark:text-slate-100">£{(formData.amount * 0.01).toFixed(2)}</span>
                    </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <InputField
                  ...
                  icon={<FaLocationPin className="text-gray-400 dark:text-slate-400" />}
              ...
              <div>
                <InputField
                  ...
                  isRequired={false}
                  variant="outline"
                  icon={<FaEdit className='text-gray-400 dark:text-slate-400' />}
                  classNames="border-b-2 border-gray-200 dark:border-slate-600 py-3 px-0"
                  error={errors.description}
                />
              </div>

              {/* Error message at bottom for form step */}
              {errors.general && (
                <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/30 text-red-600 dark:text-red-400 rounded-lg">
                  <div className="flex items-center gap-2">
                    <FiSend className="text-lg flex-shrink-0" />
