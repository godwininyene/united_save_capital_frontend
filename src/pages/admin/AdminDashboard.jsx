...
const getStatusBadge = (status) => {
  switch(status?.toLowerCase()) {
    ...
    case 'declined':
      return <span className="px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">Declined</span>;
    default:
  ...
  if (!stats && fetched) {
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        ...
        <p className="text-gray-500 mt-1">Failed to load dashboard statistics</p>
        <button 
          onClick={fetchStats}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex justify-between items-center mb-6">
            ...
            <select className="text-sm border border-gray-200 rounded-lg px-3 py-1 focus:outline-none focus:ring-2 focus:ring-primary-2/50">
    ...
    <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow">
      ...
      {change && (
        <div className={`mt-4 text-sm font-medium ${isPositive ? 'text-green-500' : 'text-red-500'}`}>
          {change} {isPositive ? '↑' : '↓'} <span className="text-gray-500 ml-1">vs last month</span>
        </div>
      )}
    </div>
  );
...
const ActivityTable = ({ title, icon, headers, data, link, emptyMessage = "No data available" }) => (
  <div className="bg-white rounded-xl shadow-sm border border-gray-100">
    <div className="p-4 border-b border-gray-100 flex justify-between items-center">
      ...
      <a href={link} className="text-sm text-primary-2 hover:underline flex items-center gap-1">
